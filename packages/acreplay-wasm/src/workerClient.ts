...
type WorkerLike = Pick<Worker, 'addEventListener' | 'removeEventListener' | 'postMessage' | 'terminate'>
...
export function createReplayWorkerClient(worker: WorkerLike): WasmReplayWorkerClient {
  ...
  const onMessageError = (event: Event) => {
    const error = event instanceof ErrorEvent ? event.error ?? event.message : new Error('Worker message error')
  ...
  const onError = (event: Event) => {
    const error = event instanceof ErrorEvent ? event.error ?? event.message : new Error('Worker error')
    for (const request of pending.values()) {
      request.reject(error)
    }
    pending.clear()
  ...
  async function parseCar(fileBytes: ArrayBuffer, carIndex: number): Promise<ParsedCarResultV1> {
    await ensureInitialized()
    const response = await sendRequest({id: createRequestId(), type: 'parseCar', fileBytes, carIndex})
    if (!response.ok) {
      throw new Error(response.error)
    }
