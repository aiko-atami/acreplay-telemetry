...
function normalizeParseCarResult(json: {
  ...
  lapPacks: { lapNumber: number; base64: string }[]
    ...
    return {
      manifest: json.manifest, lapPacks: json.lapPacks.map((lap) => ({
  ...
  export async function createReplayParser(
      factory: WasmModuleFactory,
      ): Promise<WasmReplayParser> {
    ...
    return {
      ...
      ,
        ...
        try {
          ...
          const json = readJsonResult < {manifest: ReplayManifestV1
          lapPacks: { lapNumber: number; base64: string }[]
