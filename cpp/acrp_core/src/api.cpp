...
struct CarFrameExtraV6 {
  ...
  void outputData(std::ostream& out) const {
    out << i << ',' << h << ',' << h2 << ',' << h3 << ',' << h4 << ',' << f << ',' << h5 << ',' << h6 << ',' << h7
        << ',' << h8 << ',' << h9 << ',' << h10 << ',' << h11 << ',' << h12 << ',' << f2 << ',' << h13 << ',' << h14
        << ',' << i2 << ',' << h15 << ',' << h16 << ',' << h17 << ',' << h18 << ',' << f3 << ',' << h19 << ',' << h20
        << ',' << i3 << ',' << h21 << ',' << h22 << ',' << h23 << ',' << h24 << ',' << f4 << ',' << h25 << ',' << h26
        << ',' << i4 << ',' << i5 << ',' << +b << ',' << +wipers << ',' << (status & 0b0111) << ','
        << ((status >> 3) & 0x1) << ',' << +handbrake << ',' << +b2 << ',' << +b3 << ',' << +b4 << ',' << h27 << ','
        << +clutch << ',' << +b5 << ',' << i6 << ',' << h28 << ',' << h29;
  }
};
...
class SpanStreamBuffer final : public std::streambuf {
 public:
  SpanStreamBuffer(char* data, std::size_t size) : begin_(data), end_(data + size) { setg(data, data, data + size); }
...
class SpanInputStream final : public std::istream {
 public:
  explicit SpanInputStream(std::span<const std::byte> bytes)
      : std::istream(nullptr), buffer_(reinterpret_cast<char*>(const_cast<std::byte*>(bytes.data())), bytes.size()) {
    rdbuf(&buffer_);
  }

  explicit SpanInputStream(std::span<const std::uint8_t> bytes)
      : std::istream(nullptr), buffer_(reinterpret_cast<char*>(const_cast<std::uint8_t*>(bytes.data())), bytes.size()) {
    rdbuf(&buffer_);
  }
 ...
 private:
  ...
  void transform(const std::array<std::uint8_t, 64>& chunk) {
    ...
    for (std::size_t i = 0; i < 16; ++i) {
      const auto j = i * 4;
      w[i] = (static_cast<std::uint32_t>(chunk[j]) << 24) | (static_cast<std::uint32_t>(chunk[j + 1]) << 16) |
             (static_cast<std::uint32_t>(chunk[j + 2]) << 8) | static_cast<std::uint32_t>(chunk[j + 3]);
    }
...
void skipTrackObjects(std::istream& in, const ReplayHeader& header) {
  ...
  const auto bytesPerFrame = std::streamoff{4} + (static_cast<std::streamoff>(header.numTrackObjects) * 12);
...
void outputCarFrame(std::ostream& out, const CarFrame& frame) {
  ...
  out << std::setprecision(std::numeric_limits<float>::max_digits10) << frame.position.x << ',' << frame.position.y
      << ',' << frame.position.z << ',' << std::setprecision(std::numeric_limits<float>::max_digits10)
      << frame.rotation.x << ',' << frame.rotation.y << ',' << frame.rotation.z << ',' << frame.velocity.x << ','
      << frame.velocity.y << ',' << frame.velocity.z << ','
      << std::setprecision(std::numeric_limits<float>::max_digits10) << frame.wheelStaticPosition[0].x << ','
      << frame.wheelStaticPosition[0].y << ',' << frame.wheelStaticPosition[0].z << ','
      << frame.wheelStaticPosition[1].x << ',' << frame.wheelStaticPosition[1].y << ','
      << frame.wheelStaticPosition[1].z << ',' << frame.wheelStaticPosition[2].x << ','
      << frame.wheelStaticPosition[2].y << ',' << frame.wheelStaticPosition[2].z << ','
      << frame.wheelStaticPosition[3].x << ',' << frame.wheelStaticPosition[3].y << ','
      << frame.wheelStaticPosition[3].z << ',' << std::setprecision(std::numeric_limits<float>::max_digits10)
      ...
      << frame.wheelPosition[0].y << ',' << frame.wheelPosition[0].z << ',' << frame.wheelPosition[1].x << ','
      << frame.wheelPosition[1].y << ',' << frame.wheelPosition[1].z << ',' << frame.wheelPosition[2].x << ','
      << frame.wheelPosition[2].y << ',' << frame.wheelPosition[2].z << ',' << frame.wheelPosition[3].x << ','
      << frame.wheelPosition[3].y << ',' << frame.wheelPosition[3].z << ','
      << std::setprecision(std::numeric_limits<float>::max_digits10) << frame.wheelRotation[0].x << ','
      << frame.wheelRotation[0].y << ',' << frame.wheelRotation[0].z << ',' << frame.wheelRotation[1].x << ','
      << frame.wheelRotation[1].y << ',' << frame.wheelRotation[1].z << ',' << frame.wheelRotation[2].x << ','
      << frame.wheelRotation[2].y << ',' << frame.wheelRotation[2].z << ',' << frame.wheelRotation[3].x << ','
      << frame.wheelRotation[3].y << ',' << frame.wheelRotation[3].z << ',' << frame.wheelAngularVelocity[0] << ','
      << frame.wheelAngularVelocity[1] << ',' << frame.wheelAngularVelocity[2] << ',' << frame.wheelAngularVelocity[3]
      << ',' << frame.slipAngle[0] << ',' << frame.slipAngle[1] << ',' << frame.slipAngle[2] << ','
      << frame.slipAngle[3] << ',' << frame.slipRatio[0] << ',' << frame.slipRatio[1] << ',' << frame.slipRatio[2]
      << ',' << frame.slipRatio[3] << ',' << frame.ndSlip[0] << ',' << frame.ndSlip[1] << ',' << frame.ndSlip[2] << ','
      << frame.ndSlip[3] << ',' << frame.load[0] << ',' << frame.load[1] << ',' << frame.load[2] << ',' << frame.load[3]
      << ',' << +frame.tireDirt[0] << ',' << +frame.tireDirt[1] << ',' << +frame.tireDirt[2] << ','
      << +frame.tireDirt[3] << ',' << frame.steerAngle << ',' << frame.bodyworkNoise << ',' << frame.drivetrainSpeed
      << ',' << +frame.currentLap << ',' << frame.currentLapTime << ',' << frame.lastLapTime << ',' << frame.bestLapTime
      << ',' << +frame.fuel << ',' << +frame.fuelPerLap << ',' << frame.rpm << ',' << +frame.gear << ',' << +frame.gas
      << ',' << +frame.brake << ',' << +frame.boost << ',' << +frame.damageFrontDeformation << ',' << +frame.damageFront
      << ',' << +frame.damageRear << ',' << +frame.damageLeft << ',' << +frame.damageRight << ','
      << std::setprecision(originalPrecision);

  out << static_cast<bool>((frame.status >> 12) & 0x1) << ',' << static_cast<bool>((frame.status >> 3) & 0x1) << ','
...
void appendF32Le(std::vector<std::byte>& out, float value) { appendU32Le(out, std::bit_cast<std::uint32_t>(value)); }
...
std::vector<std::byte> buildLapPack(const ParsedCarData& car, const LapSlice& lap, std::uint32_t recordingIntervalMs) {
  ...
  addFloatChannel(2, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].currentLapTime); });
  ...
  addFloatChannel(6, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].rotation.y); });
  addFloatChannel(7, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].rotation.x); });
  addFloatChannel(8, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].rotation.y); });
  addFloatChannel(9, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].rotation.z); });
  ...
  addFloatChannel(11, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].velocity.x); });
  addFloatChannel(12, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].velocity.y); });
  addFloatChannel(13, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].velocity.z); });
  ...
  addFloatChannel(16, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].steerAngle); });
  addI8Channel(17, [&](std::size_t frameIndex) {
    return static_cast<std::int8_t>(static_cast<int>(car.frames[frameIndex].gear) - 1);
  });
  addFloatChannel(18, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].rpm); });
  ...
  addFloatChannel(25,
                  [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].drivetrainSpeed); });
  addFloatChannel(
      32, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].wheelAngularVelocity[0]); });
  addFloatChannel(
      33, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].wheelAngularVelocity[1]); });
  addFloatChannel(
      34, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].wheelAngularVelocity[2]); });
  addFloatChannel(
      35, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].wheelAngularVelocity[3]); });
  addFloatChannel(36, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].slipRatio[0]); });
  addFloatChannel(37, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].slipRatio[1]); });
  addFloatChannel(38, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].slipRatio[2]); });
  addFloatChannel(39, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].slipRatio[3]); });
  addFloatChannel(40, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].ndSlip[0]); });
  addFloatChannel(41, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].ndSlip[1]); });
  addFloatChannel(42, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].ndSlip[2]); });
  addFloatChannel(43, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].ndSlip[3]); });
  addFloatChannel(44, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].load[0]); });
  addFloatChannel(45, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].load[1]); });
  addFloatChannel(46, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].load[2]); });
  addFloatChannel(47, [&](std::size_t frameIndex) { return static_cast<float>(car.frames[frameIndex].load[3]); });

  bool hasCsp = false;
  if (car.extraFramesV6.has_value()) {
    hasCsp = true;
    addU8Channel(64, [&](std::size_t frameIndex) { return (*car.extraFramesV6)[frameIndex].clutch; });
  } else if (car.extraFramesV7.has_value()) {
    hasCsp = true;
    addU8Channel(64, [&](std::size_t frameIndex) { return (*car.extraFramesV7)[frameIndex].clutch; });
  }
  ...
  const auto dataOffset = descriptorsOffset + static_cast<std::uint32_t>(channels.size() * kPackDescriptorBytes);
    ...
    if (cspOffset.has_value()) {
      ...
      while (true) {
        ...
        if (readString(in, static_cast<std::uint32_t>(kExtPerCar.size())) == kExtPerCar) {
          ...
          if (extraVersion > 0 && static_cast<std::size_t>(extraVersion) <= kExtPerCarBytesPerFrame.size()) {
      ...
      if (extraVersion >= 0 && bytesPerFrame > 0) {
        ...
        auto uncompressedSize = static_cast<uLongf>(bytesPerFrame) * static_cast<uLongf>(replay.header.numFrames);
        std::vector<std::uint8_t> uncompressedData(uncompressedSize);
        const auto status =
            uncompress(uncompressedData.data(), &uncompressedSize, compressedData.data(), compressedSize);
        if (status == Z_OK) {
          SpanInputStream extra(
              std::span<const std::uint8_t>(uncompressedData.data(), static_cast<std::size_t>(uncompressedSize)));
...
ReplayManifestV1 buildManifest(const ParsedReplayData& replay, const std::string& sourceSha256,
  ...
  manifest.recordingIntervalMs = static_cast<std::uint32_t>(std::lround(replay.header.recordingInterval));
...
void exportCarCsv(const ParsedReplayData& replay, const ParsedCarData& car, std::uint32_t carIndex,
  ...
  const auto inputStem = options.inputStem.empty() ? std::string_view("replay") : std::string_view(options.inputStem);
  if (!outPath.has_filename()) {
    if (options.driverName.empty() && replay.header.numCars > 1) {
      outPath.replace_filename(std::string(inputStem) + "_" + car.header.driverName + "." + std::string(kCsvExtension));
    } else {
    ...
    if (options.driverName.empty() && replay.header.numCars > 1) {
      outPath.replace_filename(outPath.stem().string() + "_" + car.header.driverName + outPath.extension().string());
    }
    if (!hadExtension) {
  ...
  out << "# numFrames " << car.header.numFrames << "\n# recordingInterval " << replay.header.recordingInterval << '\n'
      << kCarFrameLabel;
...
ParsedCarResult parseCar(std::span<const std::byte> replayBytes, std::uint32_t carIndex) {
  ...
  const auto recordingIntervalMs = static_cast<std::uint32_t>(std::lround(replay.header.recordingInterval));
